...
func (s *server) SayHello(ctx context.Context, in *pb.HelloRequest) (*pb.HelloReply, error) {
	...
	// name: "Ken Guru" street: "Straße des 17 Juni " age: 35 phoneNumber: "+0123456789" sex: "male"
	// return &pb.HelloReply{Name: "Ken Guru"}, nil
...
func interceptor(
	...
	if md, ok := metadata.FromIncomingContext(ctx); ok {
		if token := md.Get("authorization"); len(token) > 0 {
			...
			if claims, ok := tkn.Claims.(*CustomClaims); ok { // && tkn.Valid // uncomment this line to validate
				// fmt.Printf("Allowed: %+v\n", claims.Policy.Allowed)
				// fmt.Printf("Generalized: %+v\n", claims.Policy.Generalized)
				// fmt.Printf("Noised: %+v\n", claims.Policy.Noised)
				// fmt.Printf("Reduced: %+v\n", claims.Policy.Reduced)
				fmt.Printf("Standard Claims: %+v\n", claims.StandardClaims)
			} else {
				fmt.Println(err)
			}
			...
			//fmt.Print(h.(*pb.HelloReply).ProtoReflect().Descriptor().Fields())
